...
async function main() {
  const ans = await inquirer.prompt([{
    ...
    validate: function (inp) {
      return regex.test(inp) ? ([...inp][0] === [...inp][0].toUpperCase() ? true : "Not in PascalCase") : "Not a valid variable name"
    }
  }, {
    name: "state",
    message: "Does this component have state?",
    type: "confirm",
    default: false
  }, {
    name: "state2",
    message: "Is the state async?",
    type: "confirm",
    default: false,
    when: function (hash) {
      return !hash.state;
  ...
  const output = ans.state2 ? `import * as React from 'react';

export interface ${ans.name}Props {}
export interface ${ans.name}State {}

export class ${ans.name} extends React.Component<${ans.name}Props, ${ans.name}State> {
    constructor(props: Readonly<${ans.name}Props>) {
        super(props);
    ...
    render() {
        if (this.state/* .somePropertyName */ ){
            return (
                <div>
...
`: (ans.state ? `import * as React from 'react';
...
export class ${ans.name} extends React.Component<${ans.name}Props, {}> {
    render() {
        return (
          <div>
...
`);
  ...
  const filepath = path.join(process.cwd(), `${ans.name}.tsx`);

  console.log(`File to be written (${filepath}):
...
${output}`)
  ...
  if (!ans2.confirm) throw Error("User aborted");
